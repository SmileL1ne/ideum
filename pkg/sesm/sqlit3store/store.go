...
func (s *SQLite3Store) StoreFind(ctx context.Context, sessionID string) ([]byte, bool, error) {
	var b []byte
	row := s.db.QueryRow("SELECT data FROM sessions WHERE sessionID = $1 AND datetime('now') < expiry", sessionID)
...
func (s *SQLite3Store) StoreCommit(ctx context.Context, sessionID string, b []byte, expiry time.Time) error {
	...
	_, err := s.db.Exec("REPLACE INTO sessions (sessionID, data, expiry) VALUES($1, $2, datetime($3))", sessionID, b, formattedExpiry)
	if err != nil {
		return err
	}
...
func (s *SQLite3Store) StoreDelete(ctx context.Context, sessionID string) error {
	_, err := s.db.ExecContext(ctx, "DELETE FROM sessions WHERE sessionID = $1", sessionID)
	return err
}
